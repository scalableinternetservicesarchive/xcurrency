...
import { initORM, transaction } from './db/sql'
                ...
                if (secondUser){
                  if (secondUserFromAccount){
                    if (secondUserToAccount){
                      //update admin account
                      adminFromAccount.balance = Number(adminFromAccount.balance) + Number(exReqData.amountPay) - Number(exReq2.amountWant);
                      adminToAccount.balance = Number(adminToAccount.balance) - Number(exReqData.amountWant) + Number(exReq2.amountPay);
                      ...
                      //delete requests
                      await ExchangeRequest.createQueryBuilder().delete().from(ExchangeRequest)
