...
resource "aws_ecs_cluster" "xcurrency" {
  name = "xcurrency"
}

resource "aws_ecr_repository" "xcurrency" {
  name                 = "xcurrency"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.xcurrency.repository_url
  ecs_cluster    = aws_ecs_cluster.xcurrency.id
